...
function cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, iStart, jStart, A, B, opts) {
  ...
  //console.log(v0, A0.point, A1.point, B0.point, B1.point, iStart, jStart)
  //if (ea || eb) return
  if (lerp && (ea || eb)) {
    lerp(v0, A0.point, out.nodes[A0.next].point, epsilon)
    var a0 = mPointInPolygon(pip, v0, A, out.dA)
    var a1 = mPointInPolygon(pip, v0, B, out.dB)
    lerp(v0, A0.point, out.nodes[A0.prev].point, epsilon)
    var a2 = mPointInPolygon(pip, v0, A, out.dA)
    var a3 = mPointInPolygon(pip, v0, B, out.dB)
    lerp(v0, A1.point, out.nodes[A1.next].point, epsilon)
    var a4 = mPointInPolygon(pip, v0, A, out.dA)
    var a5 = mPointInPolygon(pip, v0, B, out.dB)
    lerp(v0, A1.point, out.nodes[A1.prev].point, epsilon)
    var a6 = mPointInPolygon(pip, v0, A, out.dA)
    var a7 = mPointInPolygon(pip, v0, B, out.dB)
    console.log(ea, ialpha < epsilon, 'a', a0, a1, a2, a3, '.', a4, a5, a6, a7)

    lerp(v0, B0.point, out.nodes[B0.next].point, epsilon)
    var b0 = mPointInPolygon(pip, v0, A, out.dA)
    var b1 = mPointInPolygon(pip, v0, B, out.dB)
    lerp(v0, B0.point, out.nodes[B0.prev].point, epsilon)
    var b2 = mPointInPolygon(pip, v0, A, out.dA)
    var b3 = mPointInPolygon(pip, v0, B, out.dB)
    lerp(v0, B1.point, out.nodes[B1.next].point, epsilon)
    var b4 = mPointInPolygon(pip, v0, A, out.dA)
    var b5 = mPointInPolygon(pip, v0, B, out.dB)
    lerp(v0, B1.point, out.nodes[B1.prev].point, epsilon)
    var b6 = mPointInPolygon(pip, v0, A, out.dA)
    var b7 = mPointInPolygon(pip, v0, B, out.dB)
    //console.log(eb, jalpha < epsilon, 'b', b0, b1, b2, b3, '.', b4, b5, b6, b7)

    //if (ialpha < epsilon ? a4 !== a5 : a2 !== a3) return // <-- works on exclude
    //if (a4 !== a5 || (!(ialpha < epsilon) && a2 !== a3)) return // <-- works on triangle
    //if (a4 !== a5 || (ea && a2 !== a3)) return
    if (a4 !== a5 || (!a4 && !a5 && !a6 && !a7)) return
  }
  for (
    var ix = iStart;
  ...
  var iNeighbor = k+1, jNeighbor = k
  if (false && ea) {
    iNeighbor = ialpha < epsilon ? iStart : A0.originalNext
    jNeighbor = -1
  }
  if (false && eb) {
    iNeighbor = -1
    jNeighbor = jalpha < epsilon ? jStart : B0.originalNext
  }

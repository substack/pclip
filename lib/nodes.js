...
function cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, iStart, jStart, A, B, opts) {
  ...
  var mpip = mPointInPolygon
  if (ialpha < epsilon) {
    if (lerp) {
      lerp(v0, A0.point, A1.point, epsilon)
      if (mpip(pip,v0,B,out.dB)) return
    } else {
      if (mpip(pip,A0.point,B,out.dB)) return
    }
    ...
    if (lerp) {
      ...
      if (mpip(pip,A1.point,B,out.dB)) return
    }
  }
  if (jalpha < epsilon) {
    if (lerp) {
      lerp(v0, B0.point, B1.point, epsilon)
      if (mpip(pip,v0,A,out.dA)) return
    } else {
      if (mpip(pip,B0.point,A,out.dA)) return
    }
    ...
    if (lerp) {
      ...
      if (mpip(pip,B1.point,A,out.dA)) return
    }
  }

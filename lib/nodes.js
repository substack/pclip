...
function cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, iStart, jStart, A, B, opts) {
  ...
  if (ea || eb) {
    ...
    var v = A1.point
    if (lerp) {
      lerp(v0,A1.point,out.nodes[A1.originalNext].point,epsilon)
      v = v0
    }
    var m0 = mPointInPolygon(pip, v, A, out.dA)
    var m1 = mPointInPolygon(pip, v, B, out.dB)
    console.log(m0,m1,ea,eb)
    if (m0 !== m1) return
  }
  for (
    var ix = iStart;

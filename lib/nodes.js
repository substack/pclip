...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var i = aStart; i < aEnd; i++) {
    ...
    var dA0A1 = distance(A0.point,A1.point)
    for (var j = bStart; j < bEnd; j++) {
      var B0 = out.nodes[j]
      var B1 = out.nodes[B0.originalNext]
      var dB0B1 = distance(B0.point,B1.point)
      // todo: this calculates the same many distances several times:
      var dA0B0 = distance(A0.point,B0.point)
      var dA0B1 = distance(A0.point,B1.point)
      var dA1B0 = distance(A1.point,B0.point)
      var dA1B1 = distance(A1.point,B1.point)
      if (Math.abs(dB0B1-dA0B0-dA0B1) < epsilon) {
        A0.border = j
      }
      if (Math.abs(dB0B1-dA1B0-dA1B1) < epsilon) {
        A1.border = j
      }
      if (Math.abs(dA0A1-dA0B0-dA1B0) < epsilon) {
        B0.border = i
      }
      if (Math.abs(dA0A1-dA0B1-dA1B1) < epsilon) {
        B1.border = i
      }
    }
  }
...
function cutPoint(out, x, a, b, A, B, opts) {
  ...
  if (out.nodes[x.next].border < 0 && out.nodes[x.prev].border < 0) {
    return
  }
  var n = x
  while (n.border >= 0) {
  ...
  if (lerp) {
    ...
    while (n.border >= 0 && n.index !== x.index) {
      n = out.nodes[n.next]
    }
    lerp(v0, out.nodes[n.prev].point, n.point, epsilon)
    var m0 = mPointInPolygon(pip, v0, X, dX)
    if (m0) return
  }
  if (alpha < epsilon) {
    return fuse(x,a)
  } else if (alpha > 1-epsilon) {
    return fuse(x,b)
  }
...
function cutEdge(out, point, A0, A1, B0, B1, iStart, jStart, A, B, opts) {
  ...
  if (lerp && Math.abs(njalpha-jalpha) < epsilon) {
    lerp(v0, point, B0.point, epsilon)
    var ring = getRing(out.nodes, ix)
    if (!pip(v0,ring)) {
      jx = out.nodes[jx].next
    }
  }

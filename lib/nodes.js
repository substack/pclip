...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var i = aStart; i < aEnd; i++) {
    ...
    for (var j = bStart; j < bEnd; j++) {
      ...
      if (X.border < 0) {
        ...
        X.border1 = i
        console.log('ELSE 0', X.point)
      } else {
        console.log('ELSE 1', X.point)
      }
    }
  }
  for (var i = bStart; i < bEnd; i++) {
    ...
    for (var j = aStart; j < aEnd; j++) {
      ...
      if (X.border < 0) {
        ...
        X.border1 = i
        console.log('ELSE 2', X.point)
      } else {
        console.log('ELSE 3', X.point)
      }
    }
  }
  for (var i = 0; i < 2; i++) {
    ...
    for (var j = start; j < end; j++) {
      ...
      if (n.border >= 0) {
        ...
        var A0 = n
        var A1 = out.nodes[A0.next]
        var B0 = out.nodes[n.border]
        var B1 = out.nodes[B0.originalNext]
        cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts)
      }
      if (n.border1 !== undefined) {
        var A0 = n
        var A1 = out.nodes[A0.next]
        var B0 = out.nodes[n.border1]
        var B1 = out.nodes[B0.originalNext]
        cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts)
      }
    }
  ...
  for (var i = 0; i < 2; i++) {
    ...
    for (var j = start; j < end; j++) {
      ...
      if (n.border === -2 && p.border === -1 && !p.intersect) continue
      if (n.border === -1 && p.border === -2 && !n.intersect) continue
      ...
      if (a.next === a.originalNext || a.next === b.prev) { // undo previous cut
        console.log('UNDO', x.index, x.point)
...
function cutPoint(out, x, a, b, A, B, opts) {
  if (x.neighbor >= 0) console.log('x.neighbor = ', x.neighbor)
...
function cutEdge(out, point, A0, A1, B0, B1, A, B, opts) {
  ...
  /*
  if (ea) {
    lerp(v0, point, ialpha < epsilon ? A1.point : A0.point, epsilon)
    console.log('A',A0.point,A1.point)
    console.log('B',B0.point,B1.point)
    var ring = getRing(out.nodes, jx)
    console.log('v0=',v0,'ring=',JSON.stringify(ring))
    if (!pip(v0,ring)) return
  }
  */
  /*

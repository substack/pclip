...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var i = aStart; i < aEnd; i++) {
    ...
    for (var j = bStart; j < bEnd; j++) {
      ...
      checkWeld(out,i,j,opts)
      checkWeld(out,i,B0.originalNext,opts)
      checkWeld(out,A0.originalNext,j,opts)
      checkWeld(out,A0.originalNext,B0.originalNext,opts)
...
function checkWeld(out, i, j, opts) {
  var epsilon = opts.epsilon || defaultEpsilon
  var distance = opts.distance
  var a = out.nodes[i], b = out.nodes[j]
  if (a.neighbor >= 0 || b.neighbor >= 0) return
  if (!(distance(a.point, b.point) < epsilon)) return
  a.intersect = true
  b.intersect = true
  a.neighbor = j
  b.neighbor = i
...
function cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, iStart, jStart, A, B, opts) {
  ...
  if (ialpha < epsilon || ialpha > 1-epsilon) return
  if (jalpha < epsilon || jalpha > 1-epsilon) return

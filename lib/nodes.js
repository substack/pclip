...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var i = 0; i < 2; i++) {
    ...
    for (var j = start; j < end; j++) {
      var n = out.nodes[j]
      if (n.border >= 0) {
        var nb = out.nodes[n.border]
        var nn = out.nodes[nb.next]
        var np = out.nodes[nb.prev]
        var nnn = out.nodes[nn.next]
        var nnp = out.nodes[nn.prev]
        if (nn.border < 0 && np.border < 0) {
          var X = i === 0 ? B : A, dX = i === 0 ? out.dB : out.dA
          var ring0 = getRing(out.nodes, n.border)
          var ring1 = getRing(out.nodes, n.index)
          lerp(v0, n.point, nn.point, epsilon)
          var m0 = pip(v0, ring0)
          lerp(v0, n.point, np.point, epsilon)
          var m1 = pip(v0, ring0)
          console.log(m0,m1)
          if (m0 === m1) continue
        } else {
      ...
      if (n.border >= 0) {
        ...
        console.log('border cut', n.point)

...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var ip = 0; ip < 2; ip++) {
    ...
    if (d === 2) { // single polygon with no holes
      ...
      for (var i = 0; i < P.length; i++) {
        var lp = P[i].length
        if (ptEq(P[i][0],P[i][lp-1],epsilon)) lp--
      ...
      for (var i = 0; i < P.length; i++) {
        for (var j = 0; j < P[i].length; j++) {
          var lp = P[i][j].length
          if (ptEq(P[i][j][0],P[i][j][lp-1],epsilon)) lp--
  ...
  mark(pointInPolygon, out.nodes, A, B, out.dA, out.dB, 0, mode === 'intersect', epsilon)
  mark(pointInPolygon, out.nodes, B, A, out.dB, out.dA, bStart, mode !== 'union', epsilon)
...
function mark(f, nodes, A, B, dA, dB, start, flip, epsilon) {
  var index = start
  if (dA === 2) {
    ...
    for (var i = 0; i < A.length; i++) {
      ...
      markEntries(nodes, index, inside)
      index += A[i].length -(ptEq(A[i][0],A[i][A[i].length-1],epsilon) ? 1 : 0)
    }
  } else if (dA === 4) {
    for (var i = 0; i < A.length; i++) {
      for (var j = 0; j < A[i].length; j++) {
        ...
        markEntries(nodes, index, inside)
        index += A[i][j].length -(ptEq(A[i][j][0],A[i][j][A[i][j].length-1],epsilon) ? 1 : 0)
      }
    }
  }
}

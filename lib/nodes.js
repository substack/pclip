...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var k = 0; k < 1; k++) {
    ...
    for (var i = xStart; i < xEnd; i++) {
      ...
      for (var j = 0; j < yEnd; j++) {
        var B0 = out.nodes[j]
        var B1 = out.nodes[B0.originalNext]
        if (A0.polygon === B0.polygon && A0.ring === B0.ring) continue
        var d0 = distance(B0.point,B1.point)
        var d1 = distance(B0.point,A0.point)
        var d2 = distance(B1.point,A0.point)
        if (!(Math.abs(d0-d1-d2) < epsilon)) continue
        if (Math.abs(d0-d1) < epsilon) continue
        if (Math.abs(d0-d2) < epsilon) continue
        //if (Math.abs(d1-d2) < epsilon) continue
        console.log(
          'b',
          A0.point.join(','), A1.point.join(','),
          B0.point.join(','), B1.point.join(','),
          ':',
          d0,d1,d2
        )
        A0.border = j
        cutEdge(out, [A0.point[0],A0.point[1]], A0, A1, B0, B1, A, B, opts, true)
      }
    }
  }

  // phase one part three: cut points on edges
  /*
  for (var i = 0; i < 2; i++) {
  ...
  */
  ...
  for (var j = bEnd; j < len; j += 2) {
    ...
    console.log(
      ...
      ':',
      [
        A0.intersect, // && A0.border < 0,
        A1.intersect, // && A1.border < 0,
        A2.intersect, // && A2.border < 0,
      ].map(x => x ? '1' : '0').join(''),
      [
        B0.intersect, // && B0.border < 0,
        B1.intersect, // && B1.border < 0,
        B2.intersect, // && B2.border < 0,
      ].map(x => x ? '1' : '0').join(''),
      [c0,c1,c2,c3,c4,c5,c6,c7].map(x => x ? '1' : '0').join(''),
      [A0.border, A1.border, A2.border, B0.border, B1.border, B2.border]
        .map(x => String(x < 0 ? '_' : x)).join(' '),
    )
    if (c0 && !c1 && !A1.intersect && !A2.intersect && !B1.intersect && !B2.intersect) {
      console.log('cut1')
      cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts, true)
    } else if (!c0 && c1 && !A1.intersect && A2.intersect && !B1.intersect && !B2.intersect) {
      console.log('cut2')
      // 101 101 01010011 true
      // 101 110 01010011 false
      cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts, true)
    } else if (!c0 && !c1 && !c7 && A1.border < 0) { // && A2.border < 0 && B1.border < 0 && B2.border < 0) {

  ...
  for (var i = 0; i < 2; i++) {
    ...
    for (var j = start; j < end; j++) {
      ...
      for (var k = 0; k < 2; k++) {
        ...
        var nn = out.nodes[n.originalNext]
        var np = out.nodes[n.originalPrev]
        var nb = out.nodes[border]
        var nbn = out.nodes[nb.originalNext]
        var nbp = out.nodes[nb.originalPrev]
        var nbnn = out.nodes[nbn.originalNext]
        var nbnp = out.nodes[nbn.originalPrev]
        ...
        var c = collinear(A0.point, A1.point, B0.point, B1.point, opts)
        if (c === 0 && A0.border >= 0 && A0.originalNext !== A0.next) {
          c = collinear(A0.point, A2.point, B0.point, B1.point, opts)
          if (c > 0) continue
        }
        ...
        console.log('x', n.point.join(','), c, nb.border, nn.border, np.border, nbn.border, nbp.border,
          nn.point.join(','), np.point.join(','), nbn.point.join(','), nbp.point.join(',')
        )

        if (a !== null && b !== null) {
          var X = i === 0 ? B : A, dX = i === 0 ? out.dB : out.dA
          var ring0 = getRing(out.nodes, border)
          var ring1 = getRing(out.nodes, n.index)
          lerp(v0, n.point, a.point, epsilon)
          var m0 = pip(v0, ring0)
          lerp(v0, n.point, b.point, epsilon)
          var m1 = pip(v0, ring0)
          console.log(m0,m1)
          if (k === 0 && m0 === m1 && c === 0) {
            console.log('continue0', nb.border, nn.border, np.border, nbn.border, nbp.border)
            continue
          }
          if (k === 1 && m0 !== m1) {
            console.log('continue1')
          }
        } else if (nbn.border >= 0 && nbp.border >= 0) {
          console.log('continue3')
          continue
        } else {
          console.log('else', n.point.join(','), nn.border, np.border, nbn.border, nbp.border, nb.border)
          //continue
        }
        if (c === 0) {
          console.log('CUT0', n.point.join(','), A0.point.join(','), A1.point.join(','), B0.point.join(','), B1.point.join(','))
          cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts, true)
        } else if (c === 1) {
          console.log('CUT1', n.point.join(','), A0.point.join(','), A1.point.join(','), B0.point.join(','), B1.point.join(','))
          //cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts, true)
        } else if (c === 2) {
          console.log('CUT2', n.point.join(','), A0.point.join(','), A1.point.join(','), B0.point.join(','), B1.point.join(','))
          if (B0.prev === B0.originalPrev && !out.nodes[B0.prev].intersect) {
            cutEdge(out, [n.point[0],n.point[1]], A0, A1, B0, B1, A, B, opts, true)
          } else {
            console.log('CUT2 skip')
          }
        } else {
          console.log('CUT ELSE')
        }
      }
    }
  }
...
function collinear(A0, A1, B0, B1, opts) {
  var distance = opts.distance
  var epsilon = opts.epsilon || defaultEpsilon
  ...
  if (Math.abs(d3-d1-d2-d4) < epsilon) {
    return 1
  } else if (Math.abs(d4-d1-d2-d3) < epsilon) {
    return 2
  } else if (Math.abs(d0-d1-d5-d4) < epsilon) {
    return 3
  } else if (Math.abs(d0-d2-d5-d3) < epsilon) {
    return 4
  }
  return 0
}

  ...
  for (var i = 0; i < 2; i++) {
    ...
    for (var j = start; j < end; j++) {
      ...
      for (var k = 0; k < 2; k++) {
        ...
        var np = out.nodes[n.prev]
        var nb = out.nodes[border]
        var nbn = out.nodes[nb.next]
        var nbp = out.nodes[nb.prev]
        var nbnn = out.nodes[nbn.next]
        var nbnp = out.nodes[nbn.prev]
        console.log('X',
          nn.border, np.border, nb.border, nbn.border, nbp.border
        )
        if (nbn.border < 0 && nbp.border < 0) {
          var X = i === 0 ? B : A, dX = i === 0 ? out.dB : out.dA
          var ring0 = getRing(out.nodes, border)
          var ring1 = getRing(out.nodes, n.index)
          lerp(v0, n.point, nbn.point, epsilon)
          var m0 = pip(v0, ring0)
          lerp(v0, n.point, nbp.point, epsilon)
          var m1 = pip(v0, ring0)
          if (k === 0 && m0 === m1) continue
          if (k === 1 && m0 !== m1) continue
        } else if (nb.border < 0) {
          continue

...
module.exports = function (out, A, B, opts, mode) {
  ...
  for (var i = aStart; i < aEnd; i++) {
    ...
    for (var j = bStart; j < bEnd; j++) {
      ...
      cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, i, j, distance, epsilon)
    }
  }

  // phase two: calculate entry/exit
  mark(pointInPolygon, out.nodes, B, out.dB, 0, mode === 'intersect', epsilon, dot)
  mark(pointInPolygon, out.nodes, A, out.dA, bStart, mode !== 'union', epsilon, dot)
...
function cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, iStart, jStart, distance, epsilon) {
  var k = out.nodes.length
  var point = [v0[0],v0[1]]
  var da0v = distance(A0,v0)
  var db0v = distance(B0,v0)
  ...
  var iNeighbor = k+1, jNeighbor = k, iNew = k, jNew = k+1
  if (ialpha < epsilon) {
    jNeighbor = jx
    jNew = k
  } else if (ialpha > 1-epsilon) {
    jNeighbor = out.nodes[jx].next
    jNew = k
  }
  if (jalpha < epsilon) {
    iNeighbor = ix
  } else if (jalpha < epsilon) {
    iNeighbor = out.nodes[ix].next
  }
  if (ialpha < epsilon || ialpha > 1-epsilon) {
    out.nodes[jNeighbor].intersect = true
    //out.nodes[jNeighbor].alpha = ialpha
    out.nodes[jNeighbor].neighbor = iNeighbor
  }
  if (jalpha < epsilon || jalpha > 1-epsilon) {
    out.nodes[iNeighbor].intersect = true
    //out.nodes[iNeighbor].alpha = jalpha
    out.nodes[iNeighbor].neighbor = jNeighbor
  }
  if (!(ialpha < epsilon || ialpha > 1-epsilon)) {
    out.nodes.push({
      point,
    ...
    out.nodes[out.nodes[ix].next].prev = iNew
    out.nodes[ix].next = iNew
  }
  if (!(jalpha < epsilon || jalpha > 1-epsilon)) {
    out.nodes.push({
      point,
    ...
    out.nodes[out.nodes[jx].next].prev = jNew
    out.nodes[jx].next = jNew
  }
}

function mark(f, nodes, B, dB, start, flip, epsilon, dot) {
  var index = start
  do {
    var n = nodes[index]
    var inside = !!(mPointInPolygon(f, n.point, B, dB) ^ flip ^ n.hole)
    markEntries(nodes, index, inside, epsilon, dot)
    index = n.nextPolygon
  } while (index !== start)
}

function markEntries(nodes, start, inside, epsilon, dot) {
  var index = start
  do {
    var n = nodes[index]
    if (n.intersect && dot && (n.alpha < epsilon || n.alpha > 1-epsilon)) {

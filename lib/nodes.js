...
module.exports = function (out, A, B, opts) {
  ...
  for (var i = 0; i < aEnd-aStart; i++) {
    var nA = out.nodes[aStart+i]
    var A0 = nA.point
    var A1 = out.nodes[nA.next].point
    var dA01 = distance(A0,A1)
    for (var j = 0; j < bEnd-bStart; j++) {
      var nB = out.nodes[bStart+j]
      var B0 = nB.point
      var B1 = out.nodes[nB.next].point
      var dB01 = distance(B0,B1)
      if (!intersect(v0, A0, A1, B0, B1)) continue
      cutEdge(out, v0, A0, A1, B0, B1, dA01, dB01, aStart+i, bStart+j, distance)
    }
  }
